...
// Tool: Export Route to Strava
server.registerTool(
  "export_route_to_strava",
  {
    description: `Export a generated route to Strava as a planned activity. Creates a GPX file from route data and uploads it to Strava.

WHEN TO USE:
- User wants to save a generated route to their Strava account
- User wants to follow a route during their run
- User approves a route and wants to export it
- Queries like: "Save this route to Strava", "Export route to Strava", "Upload this to my Strava"

WORKFLOW:
1. User generates routes with generate_running_route
2. User selects a route they like
3. Call this tool with the route ID or route data
4. Tool creates GPX file and uploads to Strava
5. Returns Strava activity URL

REQUIREMENTS:
- Strava access token (user must be authenticated)
- Route data (from generate_running_route output)

OUTPUT:
- Success confirmation
- Strava activity ID and URL
- GPX download link (as fallback)`,
    inputSchema: {
      routeId: z
        .string()
        .optional()
        .describe("Route ID from generate_running_route output (e.g., 'route-1', 'route-landmarks')"),
      routeData: z
        .object({
          name: z.string(),
          distance: z.number(),
        ...
        .optional()
        .describe("Route data object (if routeId not available, provide full route data)"),
      activityName: z
        .string()
        .optional()
        .describe("Custom name for the Strava activity (defaults to route name)"),
      description: z
        .string()
        .optional()
        .describe("Description for the Strava activity"),
      token: z
        .string()
        .optional()
        .describe("Strava access token (optional - OAuth handles authentication automatically)"),
  ...
  async ({ routeData, activityName, description, token }, extra) => {
    // Try manual token first, then OAuth
    let auth = token
      ? { userId: "manual", accessToken: token }
      : await getAuth(extra);
    ...
    try {
      // Import GPX and upload utilities
      const { generateGPX, validateGPX } = await import("./routes/gpx.js");
      const {
        uploadGPXToStrava,
        checkUploadStatus,
        getStravaActivityURL,
      } = await import("./strava-upload.js");
      ...
      if (!routeData) {
        return {
          content: [
            {
              type: "text",
              text: "❌ Error: Route data is required. Please provide the route data from generate_running_route.",
            },
          ],
          isError: true,
        };
      ...
      // Prepare GPX track points
      const trackPoints = routeData.path.map((point, index) => {
        // Try to get elevation from elevation profile
        let elevation: number | undefined;
        if (routeData.elevationProfile && routeData.elevationProfile.length > 0) {
          // Find closest elevation point
          const progress = index / routeData.path.length;
          const elevIndex = Math.floor(
            progress * routeData.elevationProfile.length
          );
          elevation = routeData.elevationProfile[elevIndex]?.elevation;
        }
      ...
      // Generate GPX
      const gpxName = activityName || routeData.name;
      const gpxDescription =
        description ||
        `${routeData.distance}km route with ${routeData.elevationGain}m elevation gain. Generated by Strava Running Coach.`;

      const gpxContent = generateGPX(trackPoints, {
        name: gpxName,
      ...
      // Validate GPX
      if (!validateGPX(gpxContent)) {
        return {
          content: [
            {
              type: "text",
              text: "❌ Error: Generated GPX file is invalid. Please try again.",
            },
          ],
          isError: true,
        };
      ...
      // Upload to Strava
      const uploadResponse = await uploadGPXToStrava(
        auth.accessToken,
        gpxContent,
        gpxName,
        gpxDescription
      ...
      const processedUpload = await checkUploadStatus(
        auth.accessToken,
        uploadResponse.id
      );

      if (!processedUpload.activity_id) {
        return {
          content: [
            {
              type: "text",
              text: "⏳ Route uploaded to Strava but still processing. Check your Strava account in a few moments.",
            },
          ],
          isError: false,
        };
      ...
      // Generate Strava URL
      const stravaUrl = getStravaActivityURL(processedUpload.activity_id);

      // Create GPX download data URL
      const gpxDataUrl = `data:application/gpx+xml;base64,${Buffer.from(gpxContent).toString("base64")}`;

      return {
        structuredContent: {
          success: true,
          stravaActivityId: processedUpload.activity_id,
          stravaUrl,
          gpxDownloadUrl: gpxDataUrl,
          routeName: gpxName,
        ...
        content: [
          {
            type: "text",
            text: `✅ **Route Exported to Strava!**\n\n**Route:** ${gpxName}\n**Distance:** ${routeData.distance}km\n**Elevation Gain:** ${routeData.elevationGain}m\n\n🔗 **View on Strava:** ${stravaUrl}\n\n💡 **Tip:** You can now view this route in your Strava account and use it for navigation during your run.`,
          },
        ],
        isError: false,
      };
      ...
      console.error("Error exporting route to Strava:", error);
      return {
        content: [
          {
            type: "text",
            text: `❌ Error exporting route to Strava: ${error instanceof Error ? error.message : "Unknown error"}. Please try again or download the GPX file manually.`,
          },
        ],
        isError: true,
      };
